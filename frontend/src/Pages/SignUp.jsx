          ...
          <form className="w-full shadow-xs shadow-gray-600 border border-stone-200 bg-amber-50/30 p-9 rounded-4xl max-w-md">  
            ...
            <input type="password" value={password} onChange={(e) => { setPassword(e.target.value) }} className="duration-400 block w-full px-10 py-3 text-gray-700 bg-white border rounded-lg  focus:border-amber-100  focus:ring-amber-200 focus:outline-none focus:ring focus:ring-opacity-40" placeholder="Password"/>
              ...
              <div className="mt-6">
                  ...
                  {Login && <Link to='/forgot'><button className="w-fit"><p className="mt-2 text-sm underline inline-block text-center">forgot password?</p></button></Link>}
                  <p className="mt-1 text-center text-gray-600 ">{Login?'or':'Or continue'}</p>
  
                  <button type="button" onClick={() => loginWithGoogle()} className="w-full hover:text-black active:text-black flex items-center justify-center px-6 py-3 mt-4 text-gray-600 transition-colors duration-300 transform border rounded-lg hover:bg-amber-100">
                    ...
                    <span className="mx-2 block">{Login ? 'Sign In' : 'Sign Up'} with Google</span>
                  </button>
